...
GROUP BY p.nombre_proyecto, t.estado
...
HAVING COUNT(a.id_asignacion) > 3;
...
SELECT 
    p.nombre_proyecto,
    ROUND(AVG(h.fecha_estimada - p.fecha_inicio), 1) AS promedio_dias_hasta_hitos
FROM proyecto p
JOIN hito h ON p.id_proyecto = h.id_proyecto
GROUP BY p.nombre_proyecto
HAVING AVG(h.fecha_estimada - p.fecha_inicio) > 0;
